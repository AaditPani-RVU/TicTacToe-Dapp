...
contract TicTacToe {
    ...
    uint public countGame = 1; 
    bool public pausedAllGames; 
    uint8 constant ARRAY_LENGTH = 3; 
    uint immutable MINIMUM_ENTRY_FEE; 
...
function startGame() public payable notPaused {
    require(msg.value == MIN_FEE, "Entry fee must be exactly 0.0001 ETH");

    // user deposits entry fee to the contract
    (bool sent, ) = payable(address(this)).call{value: msg.value}("");
    require(sent, "Failed to send Ether");

    // create and set up the new game
    Game memory game;
    ...
    require(msg.value == 0.00001 ether, "Each move costs 0.00001 ETH");
    ...
    // update rewardPool with the move fee
    game.rewardPool += msg.value;
    ...
    games[_gameId] = game; // store updated game

    // distribute rewards if the game is finished
